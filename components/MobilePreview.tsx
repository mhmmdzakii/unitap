...
export default function MobilePreview({ profile, links }: any) {
  ...
  // 🔥 JURUS PISAH ETALASE & LINK BIASA
         ...
         <div className="w-[320px] h-[650px] bg-black rounded-[48px] p-[8px] shadow-[0_20px_60px_rgba(0,0,0,0.12)] relative border border-gray-300 z-10 flex flex-col">
            ...
            <div className={`w-full h-full rounded-[40px] relative overflow-hidden transition-all duration-300 flex flex-col ${theme.bgTheme}`} style={{ ...customBgStyle, ...getFontFamily() }}>
               ...
               <div className="relative z-10 px-4 pt-16 flex flex-col items-center h-full pb-10 overflow-y-auto custom-scrollbar">
                 ...
                 {/* BIO */}
                 <p className={`text-[13px] mt-1 mb-4 text-center font-medium opacity-90 drop-shadow-md ${theme.textTheme}`} style={customTextStyle}>
                   {profile?.bio || 'Tulis bio Anda di sini...'}
                 </p>
                ...
                {/* SOCIALS */}
                 <div className={`flex items-center justify-center gap-4 mb-6 ${theme.textTheme}`} style={customTextStyle}>
                    ...
                    {!profile?.instagram && !profile?.tiktok && !profile?.x && !profile?.facebook && (
                      <Mail size={18} strokeWidth={1.5} className="opacity-50" />
                    )}
                 </div>

                 {/* ========================================================= */}
                 {/* 🔥 1. GRID ETALASE PREVIEW (2x2) 🔥 */}
                 {/* ========================================================= */}
                 {etalaseLinks.length > 0 && (
                   <div className="w-full mb-6 px-1">
                     <h3 className={`font-black text-[13px] mb-3 flex items-center gap-1.5 ${theme.textTheme}`} style={customTextStyle}>
                       <ShoppingBag size={14} /> Etalase
                     </h3>
                     <div className="grid grid-cols-2 gap-2">
                       {etalaseLinks.map((product: any) => {
                         ...
                         return (
                           // Pake div biar admin nggak kepencet ke link lain pas lagi preview
                           <div key={product.id} className={`block w-full overflow-hidden flex flex-col ${isOutline && !designConfig.colorBtn ? 'border-[1.5px] border-current bg-transparent' : theme.btnTheme} ${etalaseShapeClass}`} style={btnStyleStr}>
                             ...
                             <div className="w-full aspect-square relative bg-black/5 flex items-center justify-center overflow-hidden border-b border-black/5">
                               ...
                               {product.promo_label && (
                                 <div className="absolute top-1 left-1 bg-orange-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-sm shadow-sm uppercase tracking-tighter z-10">
                                   {product.promo_label}
                                 </div>
                               )}
                             </div>
                 ...
                 <div className="w-full flex flex-col gap-3 pb-10 px-1">
                   {regularLinks.map((link: any) => {
                     ...
                     if (cat === 'music' || url.includes('spotify') || url.includes('apple')) { cat = 'music'; if(!link.image_url) img = "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?w=400&q=80"; }
                     else if (cat === 'video' || url.includes('youtube') || url.includes('youtu.be')) { cat = 'video'; if(!link.image_url) img = `https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=400&q=80`; }
                     else if (cat === 'store' || url.includes('tokopedia') || url.includes('shopee')) { cat = 'store'; if(!link.image_url) img = "https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?w=400&q=80"; }
                     ...
                     if (isFeatured && cat !== 'standard') {
                       return (
                         <div key={link.id} className={`${baseClasses} rounded-[20px] p-2 flex flex-col gap-2`} style={btnStyleStr}>
                           <div className="w-full aspect-[2/1] rounded-[12px] bg-black/5 overflow-hidden"><img src={img} className="w-full h-full object-cover" alt="Cover" /></div>
                           <div className="px-1.5 pb-1 text-left">
                              <h4 className="font-extrabold text-[13px] truncate">{title}</h4>
                           </div>
                         </div>
                       );
                     }
                     ...
                     if (cat !== 'standard' && !isMinimal) {
                       return (
                         <div key={link.id} className={`${baseClasses} ${shapeClass} p-1.5 flex justify-between items-center`} style={btnStyleStr}>
                           <div className="w-[40px] h-[40px] rounded-[10px] bg-black/5 overflow-hidden shrink-0"><img src={img} className="w-full h-full object-cover" alt="Thumb" /></div>
                           <div className="flex-1 px-2 text-center overflow-hidden">
                              <span className="font-bold text-[12px] truncate block">{title}</span>
                              <span className="text-[10px] opacity-70 capitalize">{cat}</span>
                           </div>
                     ...
                     else if (cat === 'music') IconToRender = <Music size={18} strokeWidth={1.5} />;
                     else if (cat === 'video') IconToRender = <Video size={18} strokeWidth={1.5} />;
                     else if (cat === 'store') IconToRender = <ShoppingBag size={18} strokeWidth={1.5} />;

                     return (
                       <div key={link.id} className={`${baseClasses} ${shapeClass} py-3 px-3 flex justify-between items-center`} style={btnStyleStr}>
                          ...
                          <span className="flex-1 font-bold text-[13px] truncate text-center px-1">{title}</span>
